...
- GC9A01 在启动后显示 `VOICE: LISTENING` 与 `LID: CLOSED`；双重验证成功时依次显示
  `OPENING` 与 `OPEN`。
- GPIO1 独立闭盒按键已于 2026-08-11 在 C5 实机验证：仅在开盒状态下按下才会使 SG90
  从 150° 缓慢转回 0°，屏幕依次显示 `CLOSING` 与 `CLOSED`。
...
GPIO1 独立闭盒按键连接在 GPIO1 与 GND 之间，内部上拉、低电平有效。仅当舵机处于
开盒状态时按下才会执行闭盒。
...
闭盒按键的一端接 GPIO1，另一端接 GND；它与 GPIO24 的声纹注册按键相互独立。
